...
for MOUSE_EVENT in /dev/input/by-id/*-event-mouse; do
    if [[ -e "$MOUSE_EVENT" ]]; then
        ...
        DOCKER_CMD="sudo docker run --rm -d --name $CONTAINER_NAME -p $HTTP_PORT:80 --shm-size 512m --security-opt seccomp=unconfined -e \"SCREEN_1=5900,800,1280,Galaxy Tab $HTTP_PORT,$MOUSE_EVENT\" --device $MOUSE_EVENT $IMAGE"
